...
import { PromotionDto, PromotionType } from '../../types/api-responses';

type PromotionFormType = 'Discounted dates' | 'Service level promo' | 'Coupons and codes';

const promotionTypeDisplayMap: Record<PromotionType, PromotionFormType> = {
    'DiscountedDates': 'Discounted dates',
    'ServiceLevel': 'Service level promo',
    'Coupon': 'Coupons and codes'
};

const Promotion: React.FC = () => {
    const [selectedPromotion, setSelectedPromotion] = useState<PromotionFormType | null>(null);
    ...
    const { getPromotions, togglePromotion, deletePromotion, loading } = usePromotions();
    ...
    const handleStatusChange = async (promotionId: number, newStatus: boolean) => {
        try {
            await togglePromotion(promotionId, { isActive: newStatus });
            setPromotions(prev =>
                prev.map(p => p.id === promotionId ? { ...p, isActive: newStatus } : p)
            );
            if (newStatus) {
            ...
            showToast('Error updating promotion status. Please try again.', 'error');
        }
    };

    const handleEditClick = (promotion: PromotionDto) => {
        setEditingPromotion(promotion);
        setSelectedPromotion(promotionTypeDisplayMap[promotion.promotionType]);
    };

    const handleDeleteClick = (promotion: PromotionDto) => {
    ...
    const handleDeleteConfirm = async () => {
        ...
        try {
            ...
            showToast('Error deleting promotion. Please try again.', 'error');
        } finally {
    ...
    const tableData = promotions.map((p, index) => ({
        Name: p.name,
        Type: promotionTypeDisplayMap[p.promotionType],
    ...
    const customRenderers = {
        Status: (value: boolean, _originalIndex: number, row: any) => {
            return (
                <Toggle
                    checked={value}
        ...
        '': (_: any, _originalIndex: number, row: any) => {
            return (
                <div className={styles.tableButtonContainer}>
                    <Button
                        label="Delete"
                        onClick={() => handleDeleteClick(row.originalPromotion)}
                        noAppearance={true}
                    ...
                    <Button
                        label="Edit"
                        onClick={() => handleEditClick(row.originalPromotion)}
                        noAppearance={true}
                        backgroundColor="transparent"
    ...
    return (
        <>
            {selectedPromotion ? (
                ...
                <div className={styles.promotionPage}>
                    ...
                    <div className={styles.promotionTypes}>
                        <div className={styles.promotionCard} onClick={() => setSelectedPromotion('Discounted dates')}>
                            <svg width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M16.2666 16.8899V13" stroke="#6138E0" strokeWidth="1.5" strokeMiterlimit="10" strokeLinecap="round" strokeLinejoin="round" />
                                <path d="M23.7335 16.8899V13" stroke="#6138E0" strokeWidth="1.5" strokeMiterlimit="10" strokeLinecap="round" strokeLinejoin="round" />
                                <rect width="40" height="40" rx="14" fill="#D8CCFE" />
                                <path d="M27 14.7437C27.5333 14.7437 28 15.2802 28 15.8838V26.011C28 26.7487 27.5333 27.2853 26.9333 27.2853H13.0667C12.4667 27.2853 12 26.6817 12 26.011V15.8838C12 15.2131 12.4667 14.7437 13 14.7437H27V14.7437Z" stroke="#6138E0" strokeWidth="1.5" strokeMiterlimit="10" strokeLinecap="round" strokeLinejoin="round" />
                                <path d="M12.2666 19.4385H27.7333" stroke="#6138E0" strokeWidth="1.5" strokeMiterlimit="10" strokeLinecap="round" strokeLinejoin="round" />
                            </svg>
                    ...
                    {isLoading ? (
                        ...
                        <Table headers={headers} data={tableData} customRenderers={customRenderers} />
                    )}
                </div >
            )}
            ...
            <DeleteModal
                ...
                branchName={promotionToDelete?.name || 'Promotion'}
            />
        </>
    );
};
