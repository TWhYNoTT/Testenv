...
const buildData = (labels: string[], values: number[]) => ({
    labels,
    datasets: [
        {
            data: values,
...
const AppointmentStatistics: React.FC<Props> = ({ labels, values }) => {
    return (
        <div className={styles.appointmentStatistics}>
            <div className={styles.header}>
                <h3 className='headerText'>Appointment Statistics</h3>
            ...
            <div className={styles.chartContainer}>
                <div className={styles.chart}>
                    <Doughnut data={buildData(labels, values)} options={doughnutOptions} />
                </div>
                <ul className={styles.legends}>
                    {labels.map((label, index) => (
                        <li key={index} className={styles.legendContainer}>
                            ...
                            {label} <span className={styles.percentageValue}>{values[index]}%</span>
                        </li>
                    ))}
                </ul>
            </div>
